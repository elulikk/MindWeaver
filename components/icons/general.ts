import React from 'react';

export const generalIcons = {
  add: React.createElement('path', { d: "M12 4.5v15m7.5-7.5h-15" }),
  plus: React.createElement('path', { d: "M12 4.5v15m7.5-7.5h-15" }),
  minus: React.createElement('path', { d: "M4.5 12h15" }),
  delete: React.createElement('path', { d: "M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" }),
  check: React.createElement('path', { d: "M4.5 12.75l6 6 9-13.5" }),
  close: React.createElement('path', { d: "M6 18L18 6M6 6l12 12" }),
  settings: React.createElement('path', { d: "M4 21v-7M4 10V3M12 21v-9M12 8V3M20 21v-5M20 12V3M1 14h6M9 8h6M17 16h6" }),
  warning: React.createElement('path', { d: "M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" }),
  help: React.createElement('path', { d: "M9.529 9.988a2.502 2.502 0 1 1 5 .191A2.441 2.441 0 0 1 12 12.582V14m-.01 3.008H12M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"}),
  'markdown-help': React.createElement('path', { d: "M9.879 7.519c1.171-1.025 3.071-1.025 4.242 0 1.172 1.025 1.172 2.687 0 3.712-.203.179-.43.326-.67.442-.745.361-1.45.999-1.45 1.827v.75M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-9 5.25h.008v.008H12v-.008z" }),
  changelog: React.createElement('path', { d: "M9.75 3.104v5.714a2.25 2.25 0 0 1-.659 1.591L5 14.5M9.75 3.104c-.251.023-.501.05-.75.082m.75-.082a24.301 24.301 0 0 1 4.5 0m0 0v5.714c0 .597.237 1.17.659 1.591L19.8 15.3M14.25 3.104c.251.023.501.05.75.082M19.8 15.3l-1.57.393A9.065 9.065 0 0 1 12 15a9.065 9.065 0 0 0-6.23-.693L5 14.5m14.8.8 1.402 1.402c1.232 1.232.65 3.318-1.067 3.611A48.309 48.309 0 0 1 12 21c-2.773 0-5.491-.235-8.135-.687-1.718-.293-2.3-2.379-1.067-3.61L5 14.5" }),
  time: React.createElement('path', { d: "M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" }),
  limpiar: React.createElement('path', { d: "M864.453 386.372H604.968V135.834c0-39.533-32.049-71.582-71.582-71.582h-35.791c-39.533 0-71.582 32.049-71.582 71.582v250.538H166.527c-34.592 0-62.634 28.042-62.634 62.634 0 30.327 21.556 55.617 50.181 61.392L85.997 833.761c0 49.417 35.791 90.596 89.478 89.478 53.687-1.118 85.893-53.687 156.91-53.687 172.801 0 397.852 53.687 397.852 53.687 49.417 0 89.478-40.061 89.478-89.478l68.827-326.927c22.634-9.439 38.547-31.772 38.547-57.828-0.001-34.591-28.043-62.634-62.636-62.634zM461.803 153.73c0-29.651 24.036-53.687 53.687-53.687 29.651 0 53.687 24.036 53.687 53.687v232.642H461.803V153.73z m319.456 662.753c-11.092 41.902-31.537 70.965-70.44 70.965 0 0-197.096-49.497-355.544-53.438l41.811-142.707c2.779-9.485-2.658-19.427-12.142-22.207-9.485-2.777-19.426 2.658-22.205 12.142l-45.103 153.939c-55.562 8.478-102.763 52.27-142.161 52.27-43.62 0-67.243-33.993-53.687-70.965 13.556-36.974 74.247-305.459 74.247-305.459l641.576 0.617c-0.001 0.001-45.261 262.941-56.352 304.843z m83.194-340.633H166.527c-14.825 0-26.843-12.019-26.843-26.843 0-14.825 12.019-26.843 26.843-26.843h697.927c14.825 0 26.843 12.019 26.843 26.843s-12.019 26.843-26.844 26.843z", fill: "#e4c83f" }),
  'abrir-local': React.createElement(React.Fragment, null, React.createElement('polygon', { style:{fill:"#F9B53D"}, points:"197.816,82.912 168.728,48 58.184,48 23.272,82.912 23.272,129.456 488.728,129.456 488.728,82.912"}), React.createElement('rect', { y:"129.456", style:{fill:"#FFD07D"}, width:"512", height:"334.544"}), React.createElement('rect', { x:"52.36", y:"106.184", style:{fill:"#FFFFFF"}, width:"407.272", height:"23.272"})),
  'focus-mode': React.createElement(React.Fragment, null, 
    React.createElement('path', { d: "M256,483.702C130.444,483.702,28.297,381.556,28.297,256S130.444,28.297,256,28.297 S483.702,130.444,483.702,256S381.556,483.702,256,483.702z M256,38.297C135.958,38.297,38.297,135.958,38.297,256 c0,120.041,97.661,217.702,217.703,217.702c120.041,0,217.702-97.661,217.702-217.702C473.702,135.958,376.041,38.297,256,38.297 z" }),
    React.createElement('path', { d: "M256,302.765c-25.786,0-46.765-20.979-46.765-46.765s20.979-46.765,46.765-46.765 s46.765,20.979,46.765,46.765S281.786,302.765,256,302.765z M256,219.235c-20.272,0-36.765,16.493-36.765,36.765 c0,20.272,16.493,36.765,36.765,36.765c20.272,0,36.765-16.492,36.765-36.765C292.765,235.728,276.272,219.235,256,219.235z" }),
    React.createElement('path', { d: "M285.449,370.84c-0.217,0-0.435-0.014-0.652-0.043l-86.3-11.35c-1.315-0.173-2.507-0.86-3.314-1.913 l-52.989-69.041c-0.808-1.052-1.164-2.381-0.991-3.696l11.35-86.3c0.173-1.315,0.861-2.507,1.913-3.314l69.076-52.991 c1.051-0.806,2.381-1.162,3.694-0.99l86.301,11.35c1.315,0.173,2.509,0.862,3.316,1.916l52.954,69.074 c0.807,1.052,1.162,2.38,0.989,3.694l-11.35,86.301c-0.173,1.315-0.862,2.509-1.915,3.316l-34.521,26.461l-34.519,26.492 C287.615,370.48,286.544,370.84,285.449,370.84z M201.854,349.803l82.193,10.81l32.878-25.233l32.877-25.201l10.81-82.194 l-50.435-65.787l-82.194-10.81l-65.787,50.468l-10.81,82.193L201.854,349.803z" }),
    React.createElement('path', { d: "M365.84,231.584c-2.762,0-5-2.239-5-5V63.251c0-2.761,2.238-5,5-5s5,2.239,5,5v163.333 C370.84,229.345,368.602,231.584,365.84,231.584z" }),
    React.createElement('path', { d: "M354.49,317.885c-1.279,0-2.56-0.488-3.535-1.465c-1.953-1.952-1.953-5.118,0-7.07l115.481-115.482 c1.951-1.952,5.119-1.952,7.07,0c1.953,1.953,1.953,5.119,0,7.071L358.025,316.42C357.05,317.396,355.77,317.885,354.49,317.885z" }),
    React.createElement('path', { d: "M448.749,370.84h-163.3c-2.762,0-5-2.238-5-5s2.238-5,5-5h163.3c2.762,0,5,2.238,5,5 S451.511,370.84,448.749,370.84z" }),
    React.createElement('path', { d: "M314.631,474.972c-1.279,0-2.56-0.488-3.535-1.465L195.613,358.025c-1.953-1.952-1.953-5.118,0-7.07 c1.953-1.953,5.118-1.953,7.071,0l115.481,115.481c1.953,1.952,1.953,5.118,0,7.07 C317.19,474.483,315.91,474.972,314.631,474.972z" }),
    React.createElement('path', { d: "M146.16,453.749c-2.761,0-5-2.238-5-5v-163.3c0-2.762,2.239-5,5-5s5,2.238,5,5v163.3 C151.16,451.511,148.921,453.749,146.16,453.749z" }),
    React.createElement('path', { d: "M42.028,319.631c-1.28,0-2.559-0.488-3.536-1.465c-1.953-1.952-1.953-5.118,0-7.07l115.481-115.482 c1.953-1.952,5.118-1.952,7.071,0c1.953,1.953,1.953,5.119,0,7.071L45.564,318.166C44.587,319.143,43.308,319.631,42.028,319.631 z" }),
    React.createElement('path', { d: "M226.584,151.16H63.251c-2.761,0-5-2.239-5-5s2.239-5,5-5h163.333c2.761,0,5,2.239,5,5 S229.345,151.16,226.584,151.16z" }),
    React.createElement('path', { d: "M312.885,162.51c-1.279,0-2.56-0.488-3.535-1.464L193.867,45.564c-1.953-1.953-1.953-5.119,0-7.071 c1.953-1.952,5.118-1.952,7.071,0L316.42,153.974c1.953,1.953,1.953,5.119,0,7.071 C315.444,162.021,314.164,162.51,312.885,162.51z" })
  ),
};

export const uiIcons = {
  'new-canvas': React.createElement(React.Fragment, null, React.createElement('path', { fillRule: "evenodd", d: "M3 3.75A1.75 1.75 0 014.75 2h4.5a.75.75 0 010 1.5h-4.5a.25.25 0 00-.25.25v10.5c0 .138.112.25.25.25h10.5a.25.25 0 00.25-.25V8.5a.75.75 0 011.5 0v5.25A1.75 1.75 0 0115.25 15H4.75A1.75 1.75 0 013 13.25V3.75z", clipRule: "evenodd" }), React.createElement('path', { fillRule: "evenodd", d: "M12.75 2a.75.75 0 01.75.75v3.5h3.5a.75.75 0 010 1.5h-3.5v3.5a.75.75 0 01-1.5 0v-3.5h-3.5a.75.75 0 010-1.5h3.5v-3.5A.75.75 0 0112.75 2z", clipRule: "evenodd" })),
  import: React.createElement('path', { d: "M13.5 8H4m0-2v13a1 1 0 0 0 1 1h14a1 1 0 0 0 1-1V9a1 1 0 0 0-1-1h-5.032a1 1 0 0 1-.768-.36l-1.9-2.28a1 1 0 0 0-.768-.36H5a1 1 0 0 0-1 1Z"}),
  export: React.createElement('path', { d: "M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" }),
  download: React.createElement('path', { d: "M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" }),
  undo: React.createElement('path', { d: "M9 15L3 9m0 0l6-6M3 9h12a6 6 0 010 12h-3" }),
  redo: React.createElement('path', { d: "M15 15l6-6m0 0l-6-6m6 6H9a6 6 0 000 12h3" }),
  copy: React.createElement(React.Fragment, null, React.createElement('path', { fillRule: "evenodd", d: "M18 3a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2h-1V9a4 4 0 0 0-4-4h-3a1.99 1.99 0 0 0-1 .267V5a2 2 0 0 1 2-2h7Z", clipRule: "evenodd"}), React.createElement('path', { fillRule: "evenodd", d: "M8 7.054V11H4.2a2 2 0 0 1 .281-.432l2.46-2.87A2 2 0 0 1 8 7.054ZM10 7v4a2 2 0 0 1-2 2H4v6a2 2 0 0 0 2 2h7a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-3Z", clipRule: "evenodd"})),
  paste: React.createElement('path', { d: "M15 4h3a1 1 0 0 1 1 1v15a1 1 0 0 1-1-1H6a1 1 0 0 1-1-1V5a1 1 0 0 1 1-1h3m0 3h6m-6 5h6m-6 4h6M10 3v4h4V3h-4Z"}),
  save: React.createElement(React.Fragment, null, 
    React.createElement('path', { fill: "#90b1de", stroke: "#2c3d60", strokeWidth: "1.5", d: "M 3,88 3,7 C 3,4 4,3 7,3 l 86,0 c 3,0 4,2 4,5 l 0,85 c 0,2 0,4 -4,4 l -81,0 z" }),
    React.createElement('path', { fill: "#4e7ec2", d: "m 6,6 0,81 7,7 81,0 0,-88 z" }),
    React.createElement('path', { fill: "#999999", stroke: "#555555", strokeWidth: "2", d: "M 28 62 C 24 62 24 64 24 66 L 24 97 L 76 97 L 76 66 C 76 64 75 62 72 62 L 28 62 z M 38 66 C 45 66 44 69 44 69 L 44 87 C 44 87 45 90 38 90 C 32 90 33 87 33 87 L 33 69 C 33 69 32 66 38 66 z " }),
    React.createElement('rect', { fill: "#ff0000", fillOpacity: "0.7", width: "66", height: "10", x: "17", y: "4" }),
    React.createElement('rect', { fill: "#eeeeee", width: "66", height: "42", x: "17", y: "14" }),
    React.createElement('rect', { fill: "#2c3d60", width: "6", height: "6", x: "8", y: "9" }),
    React.createElement('path', { fill: "none", stroke: "#bbbbbb", strokeWidth: "2", d: "m 21,47 58,0 M 21,35 79,35 M 21,24 79,24" }),
    React.createElement('path', { fill: "#cf9100", stroke: "#333333", strokeWidth: "2", d: "M 46,80 90,32 c 0,0 4,1 6,4 2,2 2,5 2,5 L 55,87 44,90 z" }),
    React.createElement('path', { fill: "#eeeeee", d: "M 89,36 49,80 51,81 91,37 z" }),
    React.createElement('path', { fill: "#333333", d: "m 44,90 4,-1 -3,-4 z" })
  ),
  'mark-all-incomplete': React.createElement('path', { d: "M9.75 9.75l4.5 4.5m0-4.5l-4.5 4.5M21 12a9 9 0 11-18 0 9 9 0 0118 0z" }),
  'drag-handle': React.createElement('path', { d: "M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" }),
  frame: React.createElement('path', { d: "M3.75 3.75v4.5m0-4.5h4.5m-4.5 0L9 9M3.75 20.25v-4.5m0 4.5h4.5m-4.5 0L9 15m11.25 5.25v-4.5m0 4.5h-4.5m4.5 0L15 15m4.5-11.25V9m0-4.5h-4.5m4.5 0L15 9" }),
  theme: React.createElement('path', { d: "M12 3v2.25m6.364.386l-1.591 1.591M21 12h-2.25m-.386 6.364l-1.591-1.591M12 18.75V21m-4.773-4.227l-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z" }),
  send: React.createElement('path', { d: "M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L6 12z" }),
  hide: React.createElement('path', { d: "M3.98 8.223A10.477 10.477 0 001.934 12C3.226 16.338 7.244 19.5 12 19.5c.993 0 1.953-.138 2.863-.395M6.228 6.228A10.45 10.45 0 0112 4.5c4.756 0 8.774 3.162 10.065 7.498a10.523 10.523 0 01-4.293 5.572M15 12a3 3 0 11-6 0 3 3 0 016 0z" }),
  clear: React.createElement('path', { d: "M12.75 3.375V2.25A.75.75 0 0012 1.5h-3.75a.75.75 0 00-.75.75v1.125m10.5 0V2.25A.75.75 0 0018 1.5h-3.75a.75.75 0 00-.75.75v1.125m-3 15.75l-1.21-7.26a.75.75 0 01.24-1.002L18 7.5l-1.5-1.5-5.54 5.54a.75.75 0 01-1.002.24L3 9.75" }),
  explorer: React.createElement('path', { d: "M3.75 9.75h16.5m-16.5 0a.75.75 0 01.75-.75h15a.75.75 0 01.75.75m-16.5 0v5.625c0 .621.504 1.125 1.125 1.125h14.25c.621 0 1.125-.504 1.125-1.125V9.75M3.75 9.75L3.75 6.75A.75.75 0 014.5 6h5.25a.75.75 0 01.75.75v3" }),
  log: React.createElement('path', { d: "M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25H12" }),
  'pin-on': React.createElement('path', { fillRule: "evenodd", clipRule: "evenodd", d: "M8 7V5C8 2.79086 9.79086 1 12 1C14.2091 1 16 2.79086 16 5V7H18V19H6V7H8ZM10 5C10 3.89543 10.8954 3 12 3C13.1046 3 14 3.89543 14 5V7H10V5ZM11 14V10H13V14H11Z"}),
  'pin-off': React.createElement('path', { fillRule: "evenodd", clipRule: "evenodd", d: "M10 5C10 3.89543 10.8954 3 12 3C13.1046 3 14 3.89543 14 5V7H8V5C8 2.79086 9.79086 1 12 1C14.2091 1 16 2.79086 16 5V7H18V19H6V7H10V5Z" }),
  'chevron-left': React.createElement('path', { d: "M15.75 19.5L8.25 12l7.5-7.5" }),
  'chevron-right': React.createElement('path', { d: "M8.25 4.5l7.5 7.5-7.5 7.5" }),
  chatbot: React.createElement('path', { fillRule: "evenodd", d: "M4.848 2.771A49.144 49.144 0 0112 2.25c2.43 0 4.817.178 7.152.52 1.978.292 3.348 2.024 3.348 3.97v6.02c0 1.946-1.37 3.678-3.348 3.97a48.901 48.901 0 01-3.476.383.39.39 0 00-.297.17l-2.755 4.133a.75.75 0 01-1.248 0l-2.755-4.133a.39.39 0 00-.297-.17 48.9 48.9 0 01-3.476-.384c-1.978-.29-3.348-2.024-3.348-3.97V6.74c0-1.946 1.37-3.68 3.348-3.97zM6.75 8.25a.75.75 0 01.75-.75h9a.75.75 0 010 1.5h-9a.75.75 0 01-.75-.75zm.75 2.25a.75.75 0 000 1.5h3a.75.75 0 000-1.5h-3z", clipRule: "evenodd" }),
  wifi: React.createElement(React.Fragment, null, React.createElement('path', { d: "M4.93 13.07a8.5 8.5 0 0 1 14.14 0" }), React.createElement('path', { d: "M7.76 10.24a5 5 0 0 1 8.48 0" }), React.createElement('path', { d: "M10.59 7.41a2 2 0 0 1 2.82 0" }), React.createElement('path', { d: "M12 16.5h.01" })),
  filter: React.createElement('path', { d: "M3.75 6.75h16.5M6 12h12m-9 5.25h6" }),
};
